...
Export a single SilverStripe page (with its Elemental blocks, Userforms definitions, referenced
files/images — including ones embedded as TinyMCE shortcodes) to a downloadable file, and import
that file elsewhere to create a new page — entirely through the CMS UI, no developer/CLI access
needed. Built for moving content between separate environments (e.g. dev → UAT → prod).
...
```
cp vendor/madecurious/sitetree-import-export/docs/templates/CMSMain_Content.ss \
...
**Export**: open a page in the CMS, go to its "Content Export" tab, click **Export** — a modal
lets you choose whether to include referenced files/images and add a short description, then
queues the export as a background job. The tab's history list shows every past export (and the
file originally used to *import* the page, if it was created that way), each with a download
link once complete, a delete button, and a "stale" badge once the page's published content has
changed since that export was captured.
...
```yaml
MadeCurious\SiteTreeImportExport\Serialization\SiteTreeExporter:
